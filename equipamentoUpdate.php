<?php
    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        //depurando os campos
        if(empty($_POST['idequipamento'])) { die('reload'); }
        ...
        if($filtro == 1) {
            try {
                ...
                #$sql = $pdo->prepare("SELECT idequipamento,monitor FROM equipamento WHERE descricao = :descricao AND idequipamento <> :idequipamento");
                $sql = $pdo->prepare("SELECT idequipamento,monitor FROM equipamento WHERE descricao = :descricao AND idequipamento = :idequipamento");
                ...
                $ret = $sql->rowCount();
                    ...
                    if($ret > 0) {
                        ...
                        /*if($lin->monitor == 'T') {
                            die('Essa equipamento j&aacute; est&aacute; cadastrado.');    
                        }*/
                        
                        if($lin->monitor == 'F') {
                            die('Essa equipamento j&aacute; est&aacute; cadastrado, mas est&aacute; desativado. <a href="equipamentoActivate.php?'.$py.'='.$lin->idequipamento.'" title="Ativar equipamento">Clique para ativar.</a>');    
                        }
                    }
                ...
                //insere no banco
                $sql = $pdo->prepare("UPDATE equipamento SET descricao = :descricao,anotacao = :anotacao WHERE idequipamento = :idequipamento");
                ...
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        if($_POST['modalequipamento'] == 'T'){
                            $py = md5('idequipamento');
                            echo'<url>newEquipamento.php?'.$py.'='.$_POST['idequipamento'].'</url>';
                        } else {
                            echo'true';
                        }
                    }
